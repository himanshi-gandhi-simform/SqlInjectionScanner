...
Open the POC folder in Claude Code:

```bash
cd C:\Users\himanshi.gandhi\Desktop\SqlInjectionScanner
```

The agent is picked up automatically from `.claude/agents/sql-injection-scanner.md`.
Ask for the audit in a **fresh session**:

```
Use the sql-injection-scanner agent to audit src/VulnerableShop.Api
```

Keep `verification/` out of the conversation. If the agent reads the answer key the
run is void — start over in a new session.
